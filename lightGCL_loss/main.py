...
pop_threshold = int(num_items * 0.2)  # 前20%为popular
middle_threshold = int(num_items * 0.4)  # 接下来的20%为middle
# 剩下的60%为unpopular
unpopular_items = sorted_items[2*pop_threshold:]

# 创建pop_list，其中包含每个item的类别
pop_list = np.full_like(item_popularity, 'unpopular', dtype=object)  # 假设 your_array 是您用来存储 item 数据的数组
pop_list[sorted_items[:pop_threshold]] = 'popular'
pop_list[sorted_items[pop_threshold:2*pop_threshold]] = 'middle'
...
for uid, items in enumerate(test_labels):
    filtered_test_labels[uid] = [item for item in items if item in unpopular_items]
print('Unpop Test data processed.')
loss_list = []
loss_r_list = []
loss_s_list = []
recall_20_x = []
...
for epoch in range(epoch_no):
    ...
    for batch in tqdm(range(batch_no)):
        ...
        # feed
        optimizer.zero_grad()
        loss, loss_r, loss_s= model(uids, iids, pos, neg,pop_list,alpha = args.alpha)
    ...
    if epoch % 3 == 0 :  # test every 10 epochs
        ...
        all_recall_40 = 0
        all_ndcg_40 = 0
        all_recall_20_unpop = 0
        all_ndcg_20_unpop = 0
        all_recall_40_unpop = 0
        all_ndcg_40_unpop = 0
        for batch in tqdm(range(batch_no)):
            start = batch*batch_user
            ...
            predictions = model(test_uids_input,None,None,None,pop_list,args.alpha,test=True)
            predictions = np.array(predictions.cpu())

            unpop_predictions = np.copy(predictions)

            mask = np.ones(unpop_predictions.shape, dtype=bool)
            mask[:, unpopular_items] = False
            min_value = np.iinfo(unpop_predictions.dtype).min
            unpop_predictions[mask] = min_value
            ...
            #top@20
            recall_20_unpop, ndcg_20_unpop = metrics(test_uids[start:end],unpop_predictions,20,filtered_test_labels)
            #top@40
            recall_40_unpop, ndcg_40_unpop = metrics(test_uids[start:end],unpop_predictions,40,filtered_test_labels)

            all_recall_20_unpop+=recall_20_unpop
            all_ndcg_20_unpop+=ndcg_20_unpop
            all_recall_40_unpop+=recall_40_unpop
            all_ndcg_40_unpop+=ndcg_40_unpop
        ...
        print('-------------------------------------------')
        print('UnPOP_Test of epoch',epoch,':','Recall@20:',all_recall_20_unpop/batch_no,'Ndcg@20:',all_ndcg_20_unpop/batch_no,'Recall@40:',all_recall_40_unpop/batch_no,'Ndcg@40:',all_ndcg_40_unpop/batch_no)
        recall_20_x_unpop.append(epoch)
        recall_20_y_unpop.append(all_recall_20_unpop/batch_no)
        ndcg_20_y_unpop.append(all_ndcg_20_unpop/batch_no)
        recall_40_y_unpop.append(all_recall_40_unpop/batch_no)
        ndcg_40_y_unpop.append(all_ndcg_40_unpop/batch_no)
...
print('-------------------------------------------')
print('Final Unpop test :','Recall@20:',all_recall_20_unpop/batch_no,'Ndcg@20:',all_ndcg_20_unpop/batch_no,'Recall@40:',all_recall_40_unpop/batch_no,'Ndcg@40:',all_ndcg_40_unpop/batch_no)
recall_20_x_unpop.append('Final')
recall_20_y_unpop.append(all_recall_20_unpop/batch_no)
ndcg_20_y_unpop.append(all_ndcg_20_unpop/batch_no)
recall_40_y_unpop.append(all_recall_40_unpop/batch_no)
ndcg_40_y_unpop.append(all_ndcg_40_unpop/batch_no)
...
metric_unpop = pd.DataFrame({
    'epoch':recall_20_x_unpop,
    'recall@20':recall_20_y_unpop,
    'ndcg@20':ndcg_20_y_unpop,
    'recall@40':recall_40_y_unpop,
    'ndcg@40':ndcg_40_y_unpop
})


current_t = time.gmtime()
metric.to_csv('log/result_'+args.data+'_'+str(args.temp)+'_'+str(args.lambda1)+'_'+str(args.lambda2)+'_'+time.strftime('%Y-%m-%d_%H_%M_%S',current_t)+'_'+str(args.alpha)+'.csv')
metric_unpop.to_csv('log/unpop_result_'+args.data+'_'+str(args.temp)+'_'+str(args.lambda1)+'_'+str(args.lambda2)+'_'+time.strftime('%Y-%m-%d_%H_%M_%S',current_t)+'_'+str(args.alpha)+'.csv')

torch.save(model.state_dict(),'saved_model/saved_model_'+args.data+'_'+time.strftime('%Y-%m-%d_%H_%M_%S',current_t)+'_'+str(args.alpha)+'.pt')
torch.save(optimizer.state_dict(),'saved_model/saved_optim_'+args.data+'_'+time.strftime('%Y-%m-%d_%H_%M_%S',current_t)+'_'+str(args.alpha)+'.pt')