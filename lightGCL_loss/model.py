...
class LightGCL(nn.Module):
    ...
    def forward(self, uids, iids, pos, neg,pop_list, alpha,test=False):
        if test==True:  # testing phase
            ...
            # total loss
            loss = alpha*(loss_r_pop+loss_r_mid+loss_r_unpop)+self.lambda_1 * loss_s
            print("SSL_loss:",self.lambda_1 * loss_s)
            print("Loss:",loss)
            print("Loss_total",loss_r_total+self.lambda_1 * loss_s)

            #print('loss',loss.item(),'loss_r',loss_r.item(),'loss_s',loss_s.item())
            return loss_r_total,loss_r_total, loss_s
