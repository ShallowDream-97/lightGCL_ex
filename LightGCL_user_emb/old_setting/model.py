    ...
    def forward(self, uids, iids, pos, neg, test=False):
        if test==True:  # testing phase
            ...
            for i in range(len(uids)):
                ...
                pos_emb = pos_emb + torch.sign(pos_emb) * F.normalize(random_noise, dim=-1) * self.eps1
                
                random_noise = torch.rand_like(pos_emb, device=u_emb.device)
                neg_emb = neg_emb + torch.sign(neg_emb) * F.normalize(random_noise, dim=-1) * self.eps1
