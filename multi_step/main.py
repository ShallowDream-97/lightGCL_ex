...
pop_threshold = int(num_items * 0.333)  # 前20%为popular
middle_threshold = int(num_items * 0.667)  # 接下来的20%为middle
# 剩下的60%为unpopular
...
for epoch in range(epoch_no):
    ...
    for batch in tqdm(range(batch_no)):
        ...
        # feed
        optimizer.zero_grad()
        loss_pop,loss_mid,loss_unpop, loss_r, loss_s= model(uids, iids, pos, neg,pop_list)
        loss_pop.backward(retain_graph=True)
    ...
    if epoch % 3 == 0:  # test every 10 epochs
        ...
        for batch in tqdm(range(batch_no)):
            ...
            predictions = model(test_uids_input,None,None,None,pop_list,test=True)
...
metric.to_csv('log/result_'+args.data+'_'+str(args.temp)+'_'+str(args.lambda1)+'_'+str(args.lambda2)+'_'+time.strftime('%Y-%m-%d_%H_%M_%S',current_t)+'.csv')
