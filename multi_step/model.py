...
class LightGCL(nn.Module):
    ...
    def forward(self, uids, iids, pos, neg,pop_list, test=False):
        if test==True:  # testing phase
            ...
            for i in range(len(uids)):
                ...
                for p_item, n_item in zip(u_pos, u_neg):
                    ...
                    pos_scores = (u_emb * pos_emb).sum()
                    neg_scores = (u_emb * neg_emb).sum()
                    bpr = nn.functional.relu(1 - pos_scores + neg_scores)
                    
                    item_category = pop_list[p_item.cpu().numpy()] 
                    ...
                    loss_r_total += bpr
            print(count_pop,count_mid,count_unpop)
            
            loss_r_pop = loss_pop/count_pop
            loss_r_mid = loss_mid/count_mid
            loss_r_unpop = loss_unpop/count_unpop
                
            loss_r_total = loss_r_total/self.batch_user 
            

            # total loss
            loss_pop = loss_r_pop + self.lambda_1 * loss_s
            loss_mid = loss_r_mid + self.lambda_1 * loss_s
            loss_unpop = loss_r_unpop + self.lambda_1 * loss_s
            # loss = loss_r_total + self.lambda_1 * loss_s
            #print('loss',loss.item(),'loss_r',loss_r.item(),'loss_s',loss_s.item())
            return loss_pop, loss_mid, loss_unpop,loss_r_total, loss_s
            # return loss,loss_r_total, loss_s
